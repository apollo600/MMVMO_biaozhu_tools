...
def replace_text_in_all_txt_files(directory, target, replacement, replace_range):
    ...
    for root, _, files in os.walk(directory):
        for file_name in files:
            if file_name.endswith('.txt'):
                ...
                if frame_id < replace_range[0] or frame_id > replace_range[1]:
                    continue
...
if __name__ == "__main__":
    ...
    replace_start = int(sys.argv[3])
    replace_end = int(sys.argv[4])
    ...
    # 执行替换
    modified_files, total_replacements = replace_text_in_all_txt_files(directory_path+'/labels', target_text, replacement_text, replace_range)
    print(f"替换范围 {replace_range[0]} - {replace_range[1]}")
